...
@implementation CCTeletTextLabel (Draw)

@dynamic mutableAttributedText;
@dynamic frameRef;
@dynamic activeLink;
...
{
    if (self.activeLink && self.frameRef) {
        // 1.获取选中的link所在的位置
        NSRange linkRange = self.activeLink.linkRange;
...
- (void)drawImages
{
    ...
    for (CFIndex idx = 0; idx < numberOfLines; idx++) {
        ...
        for (CFIndex idx = 0; idx < runCount; idx++) {
            ...
            imageData.imageRect = imageFrame;
            // 4.3添加图片
            if (imageData.imageType == CCImageURLType) {
                // 初始化imageView
                UIImageView *imageView = [[UIImageView alloc] initWithFrame:imageFrame];
                [imageView sd_setImageWithURL:[NSURL URLWithString:imageData.imagePath] placeholderImage:self.defaultImage completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
                ...
                CGContextRef context = UIGraphicsGetCurrentContext();
                // 绘制图片
                CGContextDrawImage(context, imageFrame, imageData.image.CGImage);
            }
        }
    }
}
...
- (void)drawLastLineWithLineRef:(CTLineRef)lineRef
{
    ...
    if (lastLineRange.location + lastLineRange.length < (CFIndex)self.mutableAttributedText.length) {
        // 2.1获取最后一行的属性字符串
        NSMutableAttributedString *truncationString = [[self.mutableAttributedText attributedSubstringFromRange:NSMakeRange(lastLineRange.location, lastLineRange.length)] mutableCopy];
        ...
        // 2.6获取需要截断的属性
        NSDictionary *tokenAttributes = [self.mutableAttributedText attributesAtIndex:truncationAttributePosition effectiveRange:NULL];
